...
class PosNav(Node):
    ...
    def __init__(self):
        ...
        #checkpoints
        Table1 = [(0.79,0),(1,0),(1.15,0),(1.3,0)]
        Table2 = [(0.79,0),(1,0),(1.25,0),(1.3,-0.3),(1.3,-0.8),(1.3,-1)]
        Table3 = [(0.4,-0.3),(0.4,-0.5),(0.48,-0.8)]
        Table4 = [(0.4,-0.4),(0.4,-0.8),(0.4,-1.2),(0.48,-1.5)]
        Table5 = [(0.4,-0.5),(0.4,-1),(0.15,-1),(0.15,-1.5),(0.15,-2),(0.15,-2.5),(0.5,-2.5),(1,-2.5),(1.25,-2.5)]
        Table6 = [(0.8,0),(1.1,0),(1.1,-0.5),(1.4,-1),(1.4,-1.6),(2,-1.6),(2.5,-1.6),(2.7,-1.2),(3,-0.4)]
        self.Tables = [Table1,Table2,Table3,Table4,Table5,Table6]

        #,(3.2,-1),(2.2,0),(2.2,-1),(3.2,0)
    ...
    def rotatebot(self, rot_angle):
        ...
        while(((c_change_dir * c_dir_diff) > 0) or (time_passed < 1 and turnflag)and ( abs(math.degrees(cmath.phase(c_target_yaw))-math.degrees(cmath.phase(c_yaw)))>2 )):
    ...
    def move_coords_back_normal(self, to_x, to_y):
        ...
        while not stop_flag:
            ...
            new_angle = (next-yaw2angle(self.mbyaw)+360)%360
            print("new angle:",new_angle,"next angle:",next,"mbyaw:",yaw2angle(self.mbyaw))
            if (180<new_angle <355 and checked):
                self.rotatebotslow(-5)
            elif (new_angle > 5 and checked):
                new_angle = (next-yaw2angle(self.mbyaw)+360)%360
                self.rotatebotslow(5)
            self.park()
            time.sleep(0.2)
...
'''
    inp = "Go"
    while inp != "done":
        inp = input("dispenser/move_R/lidar_R/ultrasonic_R/done: ")
        #if inp == "Cal":
            #auto_nav.cal()
        if inp == "dispenser":
            if tableinp():
                print("Dispenser and communications are working")
        elif inp == "move_R":
            auto_nav.move_coords(0.45,0)
            auto_nav.move_coords(0.45,-0.45)
            auto_nav.backup()

        elif inp == "lidar_R":
            if auto_nav.check_lidar():
                print('LiDAR is working')
        elif inp == 'ultrasonic_R':
            if auto_nav.get_ultra() != '':
                print('Ultrasonic sensor on Robot is working')
